...
class _ProfileScreenState extends State<ProfileScreen>
  ...
  Future<void> _showLanguageSelector() async {
          ...
          child: Column(
            ...
            children: [
              ...
              const Padding(
                padding: EdgeInsets.symmetric(horizontal: 20),
                child: Text(
                  'Health warnings will be spoken in your selected language',
              ...
              Flexible(
                child: ListView.builder(
                  shrinkWrap: true,
                  itemCount: TtsLanguage.supportedLanguages.length,
                  itemBuilder: (context, index) {
                    final language = TtsLanguage.supportedLanguages[index];
                    final isSelected =
                        language.code ==
                        (_userProfile?.preferredLanguage ?? 'en-US');
                    return ListTile(
                      leading: isSelected
    ...
    if (selectedLanguage != null && _userProfile != null) {
      ...
      await TtsService().setLanguage(selectedLanguage);

      // Reload data to show updated language
      _loadData();
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Health alerts will now be spoken in ${TtsLanguage.getByCode(selectedLanguage)?.name ?? selectedLanguage}',
            ),
            behavior: SnackBarBehavior.floating,
          ),
        );
          ...
          child: Column(
            children: [
              ...
              ListTile(
                ...
                title: const Text('Health Alert Language'),
                subtitle: Text(
                  TtsLanguage.getByCode(
                        _userProfile?.preferredLanguage ?? 'en-US',
                      )?.name ??
                      'English (US)',
                ),
