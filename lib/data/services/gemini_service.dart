...
""";
  ...
  /// Translate text to the specified language for TTS health alerts
  Future<String?> translateText(String text, String targetLanguageCode) async {
    if (!await _initialize()) {
      debugPrint('❌ Cannot translate: Service not initialized');
    ...
    try {
      final prompt =
          """
Translate the following health alert message to $targetLanguage.
Keep it natural and conversational as it will be spoken aloud.
...
""";
      ...
      if (result != null && result.isNotEmpty) {
        debugPrint('✅ Translated to $targetLanguage: ${result.length} chars');
        return result.trim();
      }
